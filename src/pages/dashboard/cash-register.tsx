...
import { Banknote, Plus, Search, X, Download } from 'lucide-react';
...
export default function CashRegisterPage() {
  ...
  const openCreateModal = () => {
    setForm({ amount: '', description: '', category: 'Varios' });
    setShowModal(true);
  };
  ...
  const totalIncome = transactions.reduce((acc, curr) => acc + Number(curr.amount), 0);

  return (
    <DashboardLayout>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold">Caja (Ingresos)</h1>
          <p className="mt-1 text-muted-foreground">Flujo de dinero entrante</p>
        </div>
        <div className="flex flex-wrap items-center gap-4">
          <div className="rounded-lg bg-emerald-500/10 px-4 py-2 border border-emerald-500/20">
            ...
            <span className="ml-2 font-bold text-emerald-700 dark:text-emerald-300">${totalIncome.toFixed(2)}</span>
          </div>
          <button
            onClick={() => exportToCSV(filteredTransactions, 'ingresos')}
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={4} className="px-6 py-16 text-center text-muted-foreground">
                  <div className="flex flex-col items-center">
                    <Banknote className="mb-3 h-12 w-12 opacity-30 text-emerald-500" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md animate-slide-up rounded-2xl border border-border/50 bg-card p-6 shadow-2xl">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-lg font-semibold">Registrar Ingreso Manual</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required className="flex h-10 w-full rounded-lg border border-input bg-background/50 px-3 text-sm focus:ring-2 focus:ring-ring" placeholder="Ej: Aporte de capital" />
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={saving} className="flex h-10 flex-1 items-center justify-center gap-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50 transition-colors">
                  {saving ? <div className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white" /> : 'Guardar Ingreso'}
                </button>
              </div>
            </form>
          </div>
